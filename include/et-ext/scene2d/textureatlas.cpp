    ...
    if (descFile.valid()) {
      ...
      while (!(descFile.stream().eof() || descFile.stream().fail())) {
        ...
        if (token == "texture:") {
          ...
          if (!fileExists(textureName)) textureName = application().resolveFileName(textureId);
          ...
          if ((*line.begin() == '{') && (*line.rbegin() == '}')) {
            ...
            while (!parser.eof()) {
              ...
              if (aToken == "name:") {
                parser >> imageName;
                if (*imageName.begin() == '"') imageName.erase(0, 1);
                if (*imageName.rbegin() == '"') imageName.erase(imageName.length() - 1);
              } else if (aToken == "texture:") {
                parser >> textureName;
                if (*textureName.begin() == '"') textureName.erase(0, 1);
                if (*textureName.rbegin() == '"') textureName.erase(textureName.length() - 1);
              } else if (aToken == "rect:") {
                std::string sRect;
                parser >> sRect;
          ...
          if (token.length() && line.length()) log::warning("Unknown token at line %d : %s", lineNumber, token.c_str());
        }
...
const s2d::Image& TextureAtlas::image(const std::string& key) const {
  ...
  if (_images.count(key) == 0) return _emptyImage;
...
std::vector<Image> TextureAtlas::imagesForTexture(const Texture::Pointer& t) const {
  ...
  for (auto& i : _images) {
    if (i.second.texture == t) result.push_back(i.second);
  }
...
rect helper::parseRectString(std::string& s) {
  ...
  if (*s.rbegin() == '"') s.erase(s.length() - 1);
