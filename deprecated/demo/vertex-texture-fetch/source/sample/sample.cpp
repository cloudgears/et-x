...
void Sample::prepare(et::RenderContext* rc) {
  ...
  _noiseFramebuffer = rc->framebufferFactory().createFramebuffer(vec2i(256), "noise-buffer", GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0, 0);
  _noiseFramebuffer->addSameRendertarget();

  _noiseFramebuffer->renderTarget(0)->setWrap(rc, TextureWrap_Repeat, TextureWrap_Repeat);
  _noiseFramebuffer->renderTarget(1)->setWrap(rc, TextureWrap_Repeat, TextureWrap_Repeat);

  setLightPosition(unitY);
...
void Sample::loadPrograms(et::RenderContext* rc) {
  ...
  _program->setUniform("environmentTexture", 1);
  _program->setUniform("normalsTexture", 2);
  _program->setUniform("reflectionTexture", 3);
  ...
  _modelProgram->setUniform("environmentMap", 1);
  _modelProgram->setUniform("reflectionMap", 3);

  _normalsProgram = rc->programFactory().loadProgram("data/shaders/normals.program", _cache);
  _normalsProgram->setUniform("inputTexture", 0);

  _noiseProgram = rc->programFactory().loadProgram("data/shaders/noise.program", _cache);
}
...
void Sample::createGeometry(et::RenderContext* rc) {
  ...
  IndexArray ia(IndexArrayFormat_32bit, primitives::indexCountForRegularMesh(gridSize, PrimitiveType_TriangleStrips), PrimitiveType_TriangleStrips);
  ia.retain();
  ...
  _vao = rc->vertexBufferFactory().createVertexArrayObject("grid", VertexArray::Pointer(&va), BufferDrawType_Static, IndexArray::Pointer(&ia), BufferDrawType_Static);
}

void Sample::render(et::RenderContext* rc, et::Texture env, et::Texture refl, et::Framebuffer::Pointer fbo) {
  rc->renderState().bindFramebuffer(_noiseFramebuffer);

  _noiseFramebuffer->setCurrentRenderTarget(0);
  rc->renderState().bindProgram(_noiseProgram);
  _noiseProgram->setUniform("timeValue", 0.125f * mainTimerPool()->actualTime());
  rc->renderer()->fullscreenPass();

  _noiseFramebuffer->setCurrentRenderTarget(1);
  rc->renderState().bindTexture(0, _noiseFramebuffer->renderTarget(0));
  rc->renderState().bindProgram(_normalsProgram);
  rc->renderer()->fullscreenPass();
  ...
  rc->renderState().bindTexture(1, env);
  rc->renderState().bindTexture(3, refl);
  ...
  if (_shouldRenderGrid) {
    ...
    _program->setPrimaryLightPosition(_lightPosition);
    _program->setCameraProperties(cam);
    _program->setUniform("mInverseMVPMatrix", _projectorMatrix);

    rc->renderState().bindTexture(2, _noiseFramebuffer->renderTarget(1));
  ...
  if (_model.valid()) {
    ...
    _modelProgram->setPrimaryLightPosition(_lightPosition);
...
void Sample::updateProjectorMatrix() {
  ...
  for (size_t i = 0; i < _frustumLines.size(); ++i) {
    if (upperBound.distanceToPoint(_frustumLines[i]) * lowerBound.distanceToPoint(_frustumLines[i]) < 0.0f) projPoints[numPoints++] = _frustumLines[i];
  }
  ...
  if (distanceToPlane < amplitude) {
    _projectorCamera.setPosition(_projectorCamera.position() + mainPlane.normal() * (amplitude - (_belowSurface ? 2.0f : 1.0f) * distanceToPlane));
  }
  ...
  float af = std::abs(dot(mainPlane.normal(), cameraRay.direction)) < std::numeric_limits<float>::epsilon() ? 0.0f : 1.0f;
  ...
  for (size_t i = 0; i < numPoints; i++) projPoints[i] = _projectorCamera.project(mainPlane.projectionOfPoint(projPoints[i]));
...
void Sample::createFrustumGeometry(et::RenderContext* rc) {
  ...
  _frustumGeometry = rc->vertexBufferFactory().createVertexArrayObject("frustum", _frustumLinesData, BufferDrawType_Stream, IndexArray::Pointer(&ai), BufferDrawType_Static);
}
...
void Sample::setLightPosition(const vec3& l) {
  _lightPosition = normalize(l);
}
