...
MainUI::MainUI(ResourceManager& rm) {
  ...
  b->clicked.connect([this](s2d::Button*) {
    ...
    if (!fileName.empty() && fileExists(fileName)) fileSelected.invokeInMainRunLoop(fileName);
  });

  b = rm.button("Process", this);
  b->setAutolayoutRelativeToParent(vec2(0.5f, 0.0f), vec2(0.0f), vec2(1.0f, 0.0f));
  b->clicked.connect([this](s2d::Button*) { processSelected.invokeInMainRunLoop(); });
  ...
  b->clicked.connect([this](s2d::Button*) {
    ...
    if (!fileName.empty()) saveSelected.invokeInMainRunLoop(fileName);
  });
