...
void Button::buildVertices(RenderInterface::Pointer&, SceneRenderer&) {
  ...
  float contentGap = (_imageSize.x > 0.0f) && ((_currentTextSize.x > 0.0f) || (_nextTextSize.x > 0.0f)) ? (5.0f * currentScreen().scaleFactor) : 0.0f;
  ...
  if (currentTextColor.w > 0.0f) {
    ...
    if (!_currentTitleCharacters.empty()) {
      buildStringVertices(_textVertices, _currentTitleCharacters, Alignment::Center, Alignment::Center, _textOrigin + 0.5f * _maxTextSize, currentTextColor * currentAlphaScale, transform);
    }

    currentAlphaScale.w = alphaScale.w * _titleAnimator.value();
    if (_titleAnimator.running()) {
      buildStringVertices(_textVertices, _nextTitleCharacters, Alignment::Center, Alignment::Center, _textOrigin + 0.5f * _maxTextSize, currentTextColor * currentAlphaScale, transform);
    }
  }
...
void Button::setBackground(const Image& img) {
  for (uint32_t i = 0; i < State_max; ++i) _background[i] = img;
...
void Button::performClick() {
  ...
  if (currentTime > _lastClickTime + _clickTreshold) {
    ...
    if (_type == Type_CheckButton) setSelected(!_selected);
...
void Button::adjustSizeForText(const std::string& text, float duration, bool vertical, bool horizontal) {
  ...
  if (horizontal) currentSize.x = newSize.x;

  if (vertical) currentSize.y = newSize.y;
