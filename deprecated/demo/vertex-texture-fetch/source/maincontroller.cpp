...
void MainController::setRenderContextParameters(et::RenderContextParameters& p) {
  ...
  p.supportedInterfaceOrientations = InterfaceOrientation_Any & (~InterfaceOrientation_PortraitUpsideDown);
...
void MainController::applicationDidLoad(et::RenderContext* rc) {
  ...
  _gestures.pressed.connect([this](vec2, PointerType) { _sample.stopCamera(); });

  _gestures.pointerPressed.connect([this](et::PointerInputInfo i) { _pointerCaptured = _gui->pointerPressed(i); });

  _gestures.pointerMoved.connect([this](et::PointerInputInfo i) { _gui->pointerMoved(i); });
  ...
  _environmentBuffer = rc->framebufferFactory().createCubemapFramebuffer(512, "main-env", envInternal, envFormat, envType, 0, 0, 0);

  _environmentBlurredBuffer[0] = rc->framebufferFactory().createCubemapFramebuffer(256, "ds-1-env", envInternal, envFormat, envType, 0, 0, 0);

  _environmentBlurredBuffer[1] = rc->framebufferFactory().createCubemapFramebuffer(128, "ds-2-env", envInternal, envFormat, envType, 0, 0, 0);

  _environmentBlurredBuffer[2] = rc->framebufferFactory().createCubemapFramebuffer(64, "ds-3-env", envInternal, envFormat, envType, 0, 0, 0);
  ...
  if (mesh.valid()) {
    ...
    _sample.setModelToDraw(rc->vertexBufferFactory().createVertexArrayObject("model", vdata, BufferDrawType_Static, idata, BufferDrawType_Static));
  }
  // */
...
void MainController::render(et::RenderContext* rc) {
  ...
  _sample.render(rc, _environmentBlurredBuffer[2]->renderTarget(), _environmentBuffer->renderTarget(), rc->renderState().defaultFramebuffer());
