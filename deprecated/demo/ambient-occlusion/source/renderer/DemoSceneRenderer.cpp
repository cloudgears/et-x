...
void SceneRenderer::init(RenderContext* rc) {
  ...
  vec2i downsampledBufferSize((bufferSize.x + INTERLEAVE_X - 1) / INTERLEAVE_X, (bufferSize.y + INTERLEAVE_Y - 1) / INTERLEAVE_Y);

  log::info("Context size: %dx%d, downsampled size: %dx%d", bufferSize.x, bufferSize.y, downsampledBufferSize.x, downsampledBufferSize.y);
...
void SceneRenderer::setScene(s3d::Scene::Pointer aScene) {
  ...
  for (s3d::Mesh::Pointer e : meshes) _allObjects.push_back(e);
