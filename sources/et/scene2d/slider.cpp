...
Slider::Slider(Element2d* parent)
  : Element2d(parent)
  , _value(timerPool()) {
  for (size_t i = 0; i < State_max; ++i) _handleScale[i] = 1.0f;
...
void Slider::setRange(float aMin, float aMax, float duration) {
  ...
  if (oldValue < _min) setValue(_min, duration);

  if (oldValue > _max) setValue(_max, duration);
...
void Slider::buildVertices(RenderInterface::Pointer&, SceneRenderer&) {
  ...
  if (_background.texture.valid()) {
    if (_backgroundImageMode == BackgroundImageMode_Stretch) {
      ...
      buildImageVertices(_backgroundVertices, _background.texture, _background.descriptor, rect(0.5f * (mainRect.size() - _background.descriptor.size), _background.descriptor.size), finalColorValue, transform);
    } else {
  ...
  if (_sliderRight.texture.valid()) {
    ...
    rect r(valuePoint, 0.5f * mainRect.height * (1.0f - colorPlaceholdersSize), mainRect.width - valuePoint, mainRect.height * colorPlaceholdersSize);
...
void Slider::setHandleImage(const Image& img, float scale) {
  for (size_t i = 0; i < State_max; ++i) setHandleImageForState(img, scale, static_cast<State>(i));
}
