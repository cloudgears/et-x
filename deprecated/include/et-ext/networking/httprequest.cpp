...
void HTTPRequest::perform() {
  ...
  if (!_private->login.empty()) curl_easy_setopt(curl, CURLOPT_USERNAME, _private->login.c_str());

  if (!_private->password.empty()) curl_easy_setopt(curl, CURLOPT_PASSWORD, _private->password.c_str());
  ...
  if (_private->params.size() + _private->uploadFiles.size() > 0) {
    ...
    for (const auto& kv : _private->params) {
      curl_formadd(&params, &lastptr, CURLFORM_COPYNAME, kv.first.c_str(), CURLFORM_PTRCONTENTS, kv.second.data(), CURLFORM_CONTENTSLENGTH, kv.second.size(), CURLFORM_END);
    }

    for (const auto& kv : _private->uploadFiles) {
      curl_formadd(&params, &lastptr, CURLFORM_COPYNAME, kv.first.c_str(), CURLFORM_FILE, kv.second.c_str(), CURLFORM_END);
    }
  ...
  if (_private->response.invalid()) _private->response = HTTPRequestResponsePointer::create();
  ...
  if (_private->_succeeded) curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &_private->response->_responseCode);
