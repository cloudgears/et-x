...
#include <et-ext/scene2d/label.hpp>
...
void Label::buildVertices(RenderInterface::Pointer&, SceneRenderer&) {
  ...
  if (_backgroundColor.w > std::numeric_limits<float>::epsilon()) buildColorVertices(_backgroundVertices, rect(vec2(0.0f), size()), _backgroundColor * alphaScale, transform);
  ...
  if (_animatingText) {
    ...
    if (hasShadow) {
      ...
      buildStringVertices(_vertices, _charListText, textHorizontalAlignment(), textVerticalAlignment(), shadowOffset, shadowColor, transform, _lineInterval);

      shadowColor.w = _shadowColor.w * alphaScale.w * fadeIn;
      buildStringVertices(_vertices, _charListNextText, textHorizontalAlignment(), textVerticalAlignment(), shadowOffset, shadowColor, transform, _lineInterval);
    }

    if (fadeOut > 0.0f) {
      buildStringVertices(_vertices, _charListText, textHorizontalAlignment(), textVerticalAlignment(), textOffset, finalColorValue * vec4(1.0, fadeOut), transform, _lineInterval);
    }

    if (fadeIn > 0.0f) {
      buildStringVertices(_vertices, _charListNextText, textHorizontalAlignment(), textVerticalAlignment(), textOffset, finalColorValue * vec4(1.0, fadeIn), transform, _lineInterval);
    }
  } else {
    if (hasShadow) {
      shadowColor.w = _shadowColor.w * alphaScale.w;
      buildStringVertices(_vertices, _charListText, textHorizontalAlignment(), textVerticalAlignment(), textOffset + _shadowOffset, shadowColor, transform, _lineInterval);
    }

    buildStringVertices(_vertices, _charListText, textHorizontalAlignment(), textVerticalAlignment(), textOffset, finalColorValue, transform, _lineInterval);
  }
...
void Label::setText(const std::string& aText, float duration) {
  ...
  if (_autoAdjustSize) setSize(_textSize);
...
vec2 Label::textSize() {
  if (!contentValid()) {
    ...
    if (_animatingText) _nextTextSize = font()->measureStringSize(_nextText.cachedText, fontSize(), fontSmoothing());
  }
...
void Label::adjustSize() {
  ...
  if (_animatingText) _textSize = maxv(_textSize, font()->measureStringSize(_nextText.cachedText, fontSize(), fontSmoothing()));
...
std::string Label::fitStringToWidthWithFont(std::string oldText, Font::Pointer font, float fontSize, float width) {
  ...
  if (std::abs(width) < minimalWidthToFit) return oldText;
  ...
  while (oldText.size()) {
    ...
    if (wsPos == std::string::npos) {
      ...
      if (measuredSize.x > width) {
        while (isWhitespaceChar(newText.back())) newText.erase(newText.end() - 1);
        newText.append("\n");
      }
    ...
    if (!isNewLineChar(nextCharStr[0])) appended.append(nextCharStr);
