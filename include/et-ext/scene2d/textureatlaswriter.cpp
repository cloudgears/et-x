...
#include <et/scene2d/textureatlaswriter.hpp>
#include <fstream>
...
bool TextureAtlasWriter::placeImage(TextureDescription::Pointer image, TextureAtlasItem& item) {
  ...
  if (item.images.size() == 0) {
    ...
    if (desc.origin.x + desc.size.x > item.maxWidth) item.maxWidth = static_cast<int>(desc.origin.x + desc.size.x) - offset.x;

    if (desc.origin.y + desc.size.y > item.maxHeight) item.maxHeight = static_cast<int>(desc.origin.y + desc.size.y) - offset.y;
  ...
  for (TextureAtlasItemList::iterator i = _items.begin(), e = _items.end(); i != e; ++i, ++textureIndex) {
    ...
    for (const auto& ii : i->images) {
        ...
        while (params.length()) {
          ...
          if (token == "offset-") {
            ...
            for (uint32_t q = 0; q < 4; ++q) {
              if (offset[q] < 1.0f) {
                offset[q] *= (q % 2 == 0) ? static_cast<float>(image.size.x) : static_cast<float>(image.size.y);
              }
            }
          } else {
      ...
      if (image.format == TextureFormat::RGBA8) {
        ImageOperations::transfer(image.data, image.size, 4, data, i->texture->size, 4, vec2i(static_cast<int>(ii.place.origin.x), static_cast<int>(ii.place.origin.y)));
      } else {
