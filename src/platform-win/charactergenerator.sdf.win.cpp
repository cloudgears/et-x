...
public:
	...
	void renderCharacter(int value, const vec2i& offset, const vec2i& size, 
		HFONT font, BinaryDataStorage& data);
...
CharDescriptor CharacterGenerator::generateCharacter(int value, CharacterFlags flags)
{
	...
	SelectObject(_private->commonDC, isBold ? _private->_boldFont : _private->_font);
	GetTextExtentPointW(_private->commonDC, string, 1, &characterSize);

	vec2i charSize = vec2i(static_cast<int>(characterSize.cx),
		static_cast<int>((value == ET_NEWLINE) || (value == ET_RETURN) ? baseSize() : characterSize.cy));
	...
	if (charSize.square() > 0)
	{
		...
		_private->renderCharacter(value, extent / 2, canvasSize, isBold ? _private->_boldFont : _private->_font, dataToProcess);
...
{
	int intSize = static_cast<int>(baseFontSize);

	commonDC = CreateCompatibleDC(nullptr);

	_font = CreateFont(-intSize, 0, 0, 0, FW_NORMAL, 0, 0, 0, DEFAULT_CHARSET, OUT_RASTER_PRECIS,
		CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, FF_DONTCARE, face.c_str());

	_boldFont = CreateFont(-intSize, 0, 0, 0, FW_BOLD, 0, 0, 0, DEFAULT_CHARSET, OUT_RASTER_PRECIS,
...
void CharacterGeneratorPrivate::renderCharacter(int value,
	const vec2i& offset, const vec2i& size, HFONT font, BinaryDataStorage& data)
{
	RECT r = { offset.x, offset.y, size.x - offset.x, size.y };
	wchar_t string[2] = { static_cast<wchar_t>(value), 0 };
	...
	BinaryDataStorage rgbData(4 * size.square(), 0);
	GetDIBits(dc, bitmap, 0, size.y, rgbData.binary(), &bitmapInfo, DIB_RGB_COLORS);
	...
	{
		uint32_t& value = *ptr++;
		data[k++] = (((value & 0x000000ff) + ((value & 0x0000ff00) >> 8) + ((value & 0x00ff0000) >> 16)) / 3) & 0xff;
	}
