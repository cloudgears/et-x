...
GameCenter::GameCenter() {
  ...
  if (fileExists(_private->optionsFileName)) {
    ...
    if (vc == ValueClass_Dictionary) _private->options = opts;
  }

  authenticate();
}

GameCenter::~GameCenter(){ET_PIMPL_FINALIZE(GameCenter)}
...
#if (TARGET_OS_IPHONE)
      UIViewController* mainViewController = (__bridge UIViewController*)reinterpret_cast<void*>(application().renderingContextHandle());
...
void GameCenter::showLeaderboard(const std::string& ldId) {
  if ([[GKLocalPlayer localPlayer] isAuthenticated]) {
    [[SharedGameCenterDelegate instance] performSelectorOnMainThread:@selector(showLeaderboard:) withObject:[NSString stringWithUTF8String:ldId.c_str()] waitUntilDone:NO];
  }
}

void GameCenter::showLeaderboards() {
  if ([[GKLocalPlayer localPlayer] isAuthenticated]) {
    [[SharedGameCenterDelegate instance] performSelectorOnMainThread:@selector(showLeaderboards) withObject:nil waitUntilDone:NO];
  }
}
...
void GameCenter::showAchievements() {
  if ([[GKLocalPlayer localPlayer] isAuthenticated]) {
    [[SharedGameCenterDelegate instance] performSelectorOnMainThread:@selector(showAchievements) withObject:nil waitUntilDone:NO];
  } else {
    authenticate();
  }
}
...
void GameCenterPrivate::saveOptions() {
  ...
  if (rename(tempFileName.c_str(), optionsFileName.c_str())) {
    ...
    if (rename(tempFileName.c_str(), optionsFileName.c_str())) log::error("Unable to save Game Center options to file %s", optionsFileName.c_str());
  }
}

std::string GameCenterPrivate::hashForScore(const Dictionary& score) {
  return "score-" + score.stringForKey(kLeaderboardId)->content + "-" + intToStr(score.integerForKey(kValueLow)->content) + "-" + intToStr(score.integerForKey(kValueHigh)->content);
}
...
#endif
            if ([gcController respondsToSelector:@selector(setLeaderboardIdentifier:)]) gcController.leaderboardIdentifier = leaderboardId;
