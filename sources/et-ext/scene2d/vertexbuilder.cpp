...
void buildQuad(SceneVertexList& vertices, const SceneVertex& topLeft, const SceneVertex& topRight, const SceneVertex& bottomLeft, const SceneVertex& bottomRight) {
...
void buildQuad(SceneVertexList& vertices, SceneVertex topLeft, SceneVertex topRight, SceneVertex bottomLeft, SceneVertex bottomRight, const mat4& m) {
...
void buildStringVertices(SceneVertexList& vertices, const CharDescriptorList& chars, Alignment hAlign, Alignment vAlign, const vec2& pos, const vec4& color, const mat4& transform, float lineInterval) {
  ...
  for (const CharDescriptor& desc : chars) {
    ...
    if ((desc.value == ET_NEWLINE) || (desc.value == ET_RETURN)) {
      ...
      buildQuad(vertices, SceneVertex(floorv(transform * topLeft), vec4(topLeftUV, sdfParameters), charColor), SceneVertex(floorv(transform * topRight), vec4(topRightUV, sdfParameters), charColor),
        SceneVertex(floorv(transform * bottomLeft), vec4(bottomLeftUV, sdfParameters), charColor), SceneVertex(floorv(transform * bottomRight), vec4(bottomRightUV, sdfParameters), charColor));
...
void buildImageVertices(SceneVertexList& vertices, const Texture::Pointer& tex, const ImageDescriptor& desc, const rect& p, const vec4& color, const mat4& transform) {
  ...
  buildQuad(vertices, SceneVertex(transform * centerTopLeft, vec4(centerTopLeftUV, mask), color), SceneVertex(transform * centerTopRight, vec4(centerTopRightUV, mask), color),
    SceneVertex(transform * centerBottomLeft, vec4(centerBottomLeftUV, mask), color), SceneVertex(transform * centerBottomRight, vec4(centerBottomRightUV, mask), color));

  if (hasLeftTopCorner) {
    buildQuad(vertices, SceneVertex(transform * topLeft, vec4(topLeftUV, mask), color), SceneVertex(transform * topCenterTopLeft, vec4(topCenterTopLeftUV, mask), color), SceneVertex(transform * leftCenterTopLeft, vec4(leftCenterTopLeftUV, mask), color),
      SceneVertex(transform * centerTopLeft, vec4(centerTopLeftUV, mask), color));
  }

  if (hasRightTopCorner) {
    buildQuad(vertices, SceneVertex(transform * topCenterTopRight, vec4(topCenterTopRightUV, mask), color), SceneVertex(transform * topRight, vec4(topRightUV, mask), color), SceneVertex(transform * centerTopRight, vec4(centerTopRightUV, mask), color),
      SceneVertex(transform * rightCenterTopRight, vec4(rightCenterTopRightUV, mask), color));
  }

  if (hasLeftBottomCorner) {
    buildQuad(vertices, SceneVertex(transform * leftCenterBottomLeft, vec4(leftCenterBottomLeftUV, mask), color), SceneVertex(transform * centerBottomLeft, vec4(centerBottomLeftUV, mask), color),
      SceneVertex(transform * bottomLeft, vec4(bottomLeftUV, mask), color), SceneVertex(transform * bottomCenterBottomLeft, vec4(bottomCenterBottomLeftUV, mask), color));
  }

  if (hasRightBottomCorner) {
    buildQuad(vertices, SceneVertex(transform * centerBottomRight, vec4(centerBottomRightUV, mask), color), SceneVertex(transform * rightCenterBottomRigth, vec4(rightCenterBottomRigthUV, mask), color),
      SceneVertex(transform * bottomCenterBottomRigth, vec4(bottomCenterBottomRigthUV, mask), color), SceneVertex(transform * bottomRight, vec4(bottomRightUV, mask), color));
  }
  ...
  if (hasTopSafe) {
    ...
    buildQuad(vertices, SceneVertex(transform * tl, vec4(tlUV, mask), color), SceneVertex(transform * tr, vec4(trUV, mask), color), SceneVertex(transform * bl, vec4(blUV, mask), color), SceneVertex(transform * br, vec4(brUV, mask), color));
  }
...
void buildColorVertices(SceneVertexList& vertices, const rect& p, const vec4& color, const mat4& transform) {
  ...
  buildQuad(vertices, SceneVertex(transform * topLeft, texCoord[0], color), SceneVertex(transform * topRight, texCoord[1], color), SceneVertex(transform * bottomLeft, texCoord[2], color), SceneVertex(transform * bottomRight, texCoord[3], color));
}
