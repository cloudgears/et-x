...
#include <et-ext/scene2d/imageview.hpp>
...
ImageView::ImageView(Element2d* parent, const std::string& name)
  ...
  , _descriptor(timerPool())
  , _backgroundColorAnimator(timerPool())
...
ImageView::ImageView(const Texture::Pointer& texture, Element2d* parent, const std::string& name)
  : Element2d(parent, ET_S2D_PASS_NAME_TO_BASE_CLASS)
  , _texture(texture)
  , _descriptor(ImageDescriptor(texture), timerPool())
  , _backgroundColorAnimator(timerPool())
  , _contentMode(ImageView::ContentMode_Stretch) {
  connectEvents();
...
ImageView::ImageView(const Texture::Pointer& texture, const ImageDescriptor& i, Element2d* parent, const std::string& name)
  : Element2d(parent, ET_S2D_PASS_NAME_TO_BASE_CLASS)
  , _texture(texture)
  , _descriptor(i, timerPool())
  , _backgroundColorAnimator(timerPool())
  , _contentMode(ImageView::ContentMode_Stretch) {
  connectEvents();
...
ImageView::ImageView(const Image& img, Element2d* parent, const std::string& name)
  ...
  , _descriptor(img.descriptor, timerPool())
  , _backgroundColorAnimator(timerPool())
  , _contentMode(ImageView::ContentMode_Stretch) {
  connectEvents();
  setSize(_descriptor.value().size, 0.0f);
    ...
    if (_contentMode == ContentMode_Tile) {
      ...
      for (size_t v = 0; v < repeatsHeight; ++v) {
        for (size_t u = 0; u < repeatsWidth; ++u) {
          ...
          buildImageVertices(_vertices, _texture, _descriptor.value(), rect(vec2(fx, fy), _descriptor.value().size), finalColor(), transform);
        }
      }
    } else {
      auto frame = calculateImageFrame();
      buildImageVertices(_vertices, _texture, frame, rect(_actualImageOrigin, _actualImageSize), finalColor(), transform);
    }
  }
  ...
  switch (_contentMode) {
    ...
    case ContentMode_Fill: {
      ...
      if (_contentMode == ContentMode_Fill) {
        ...
        if (_contentMode == ContentMode_Fit) minScale = std::min(minScale, 1.0f);
