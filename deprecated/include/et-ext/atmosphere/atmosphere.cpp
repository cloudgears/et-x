...
void Atmosphere::generateGeometry(RenderInterface::Pointer& rc) {
  ...
  for (uint32_t i = 0, e = va->size(); i < e; ++i) pos[i] = normalize(pos[i]);
...
#define SCATTERING_INTEGRAL_FUNCTION                                                                           \
  vec3 solveScatteringIntegral(in vec3 vSamplePoint, in vec3 vRay, in float fFar, in float fStartOffset) {     \
    ...
    vec3 color = vec3(0.0);                                                                                    \
    for (int i = 0; i < nSamples; i++) SCATTERING_INTEGRAL_FUNCTION_INTERIOR                                   \
    return color;                                                                                              \
  }                                                                                                            \
 ...
 *
 */
const std::string atmospherePerVertexVS = ET_TO_CONST_CHAR(ATMOSPHERE_UNIFORMS SCALE_FUNCTION SCATTERING_INTEGRAL_FUNCTION

                                                             etVertexIn vec3 Vertex;

                                                           etVertexOut vec3 vVertex; etVertexOut vec3 vDirection; etVertexOut vec3 aFrontColor; etVertexOut vec3 aSecondaryColor; etVertexOut vec3 aSourceColor;
...
const std::string atmospherePerVertexFS = ET_TO_CONST_CHAR(ATMOSPHERE_UNIFORMS

                                                             etFragmentIn etHighp vec3 vVertex;
                                                           etFragmentIn etHighp vec3 vDirection; etFragmentIn etHighp vec3 aFrontColor; etFragmentIn etHighp vec3 aSecondaryColor;
 ...
 *
 */
const std::string planetPerVertexVS = ET_TO_CONST_CHAR(ATMOSPHERE_UNIFORMS SCALE_FUNCTION SCATTERING_INTEGRAL_FUNCTION

                                                         etVertexIn vec3 Vertex;
                                                       etVertexOut vec3 vVertexWS; etVertexOut vec3 aColor;
                                                       ...
                                                       void main(void) {
                                                         ...
                                                         vec3 skyToGoundColor = solveScatteringIntegral(vVertexWS, vPrimaryLight, 0.5 * (d - b), scale(dot(vPrimaryLight, Vertex)));
...
const std::string planetPerVertexSimpleVS = ET_TO_CONST_CHAR(ATMOSPHERE_UNIFORMS
...
const std::string planetPerVertexFS = ET_TO_CONST_CHAR(etFragmentIn etHighp vec3 aColor;

                                                       void main(void) { etFragmentOut = vec4(1.0 - exp(-aColor), 1.0); });
